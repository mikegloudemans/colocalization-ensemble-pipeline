...
You'll also need to make two small changes in the file `scripts/dispatch.py`.
Find the spot in the file containing the following code:
...
In the above code, modify `/users/mgloud/projects/brain_gwas/output` to be the location of the directory where
you'd like to place your output files, and set `/users/mgloud/projects/brain_gwas/tmp` to be the
directory where you'll store temporary files (these files will be removed at the end of each run).
...
It is okay to have additional columns beyond the required ones; these will be ignored when the program is run.
...
(relative to the output file location specified in the `dispatch.py` script).
...
to be specified in the `eqtl_experiments` and `gwas_experiments`.
