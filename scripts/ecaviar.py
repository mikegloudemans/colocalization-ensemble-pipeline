...
def run_ecaviar(locus, window=500000):
    ...
    command = '/srv/persistent/bliu2/tools/caviar/CAVIAR-C++/eCAVIAR \
              ...
              -c 1 \
              -r 0.95 > /dev/null'.format(locus.gwas_suffix, locus.chrom, locus.pos, locus.eqtl_suffix, locus.gene, conditional_level)
    subprocess.check_call(command, shell=True)

    # Parse eCAVIAR results to compute CLPP score
    command = "awk '{{sum += $2}} END {{print sum}}' ../tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_ecaviar_results_col".format(locus.gwas_suffix, locus.chrom, locus.pos, locus.eqtl_suffix, locus.gene, conditional_level) 
    clpp = float(subprocess.check_output(command, shell=True))
    
    snps_tested = int(check_output("wc -l ../tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_ecaviar_results_col".format(locus.gwas_suffix, locus.chrom, locus.pos, locus.eqtl_suffix, locus.gene, conditional_level), shell=True))
    ...
    with open("{0}/{1}_ecaviar_clpp_status.txt".format(locus.basedir, locus.gwas_suffix.replace(".", "_")), "a") as a:
        a.write("{0}_{1}\t{2}\t{3}\t{4}\t{5}\t{6}\n".format(locus.chrom, locus.pos, locus.eqtl_suffix, locus.gene, conditional_level, snps_tested, clpp))
