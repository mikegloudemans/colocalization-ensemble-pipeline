...
get_gene_annotation <- function(gene_annot_file_name, chrom, gene_types=c('protein_coding', 'pseudogene', 'lincRNA',)){
  gene_df <- read.table(gene_annot_file_name, header = TRUE, stringsAsFactors = FALSE) %>% filter((chr == chrom)) # & gene_type %in% gene_types) # modified to work on simulated data
  gene_df
}
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
  ...
  # if (is.logical(covariates_file)) {adjust_covariates = as.logical(covariates_file)} else {adjust_covariates = TRUE} # modified to make adjusting covariates optional
  adjust_covariates = FALSE # for simulations
  if (adjust_covariates) {covariates_df <- get_covariates(covariates_file, samples)}
  ...
  for (i in 1:n_genes) {
    ...
    if (ncol(cis_gt) >= 2) {
      ...
      if (adjust_covariates) { # modified to make adjusting for covariates optional
