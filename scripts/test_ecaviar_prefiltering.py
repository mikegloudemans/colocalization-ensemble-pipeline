...
use_prefiltering = sys.argv[7] == "True"
...
base_output_dir = "/users/mgloud/projects/brain_gwas/output/prefiltering_test/{0}/{1}{2}".format(gwas_suffix.replace(".", "_"), adapt_dir, threshold_directory)
...
def main():
	...
	subprocess.call("mkdir /users/mgloud/projects/brain_gwas/output/prefiltering_test/{0}".format(gwas_suffix.replace(".", "_")), shell=True)
	if not adaptive_threshold:
		subprocess.call("mkdir /users/mgloud/projects/brain_gwas/output/prefiltering_test/{0}/{1}".format(gwas_suffix.replace(".", "_"), adapt_dir), shell=True)
...
def gene_by_eqtl_comparison(gwas_chrom, gwas_pos, gene, eqtls, gwas_table, gwas_suffix, coloc_status, tissue_prefix, pval_threshold, current_level):
	...
	# For now, let's just limit our tests to genes with fewer than
	# this many SNPs to avoid taking ridiculously long times.
	if combined.shape[0] > 1200:
		return False

	if not use_prefiltering:
		run_ecaviar(combined, gwas_chrom, gwas_pos, tissue_prefix, gene, current_level, False)
	else:
		run_ecaviar(ecaviar_set, gwas_chrom, gwas_pos, tissue_prefix, gene, current_level, True)
...
def run_ecaviar(combined, gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, used_prefiltering):
	...
	subprocess.call("rm /users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_ecaviar_results*".format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering), shell=True)
	...
	with open("/users/mgloud/projects/brain_gwas/tmp/vcftools/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}.txt".format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering) , "w") as w:
		snps.to_csv(w, index=False, header=False, sep="\t")

	# Pre-filter with VCFtools if we haven't already pre-filtered this site
	if "{0}_{1}".format(gwas_chrom, gwas_pos) not in os.listdir("/users/mgloud/projects/brain_gwas/data/prefiltered_vcfs/{0}".format(gwas_suffix.replace(".", "_"))): #or "{0}_{1}_{2}_prefiltered.recode.vcf".format(gwas_chrom, gwas_pos, use_prefiltering) not in "/users/mgloud/projects/brain_gwas/data/prefiltered_vcfs/{0}/{1}_{2}".format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos):
		raise Exception("Site is not prefiltered.")
		#subprocess.call("mkdir /users/mgloud/projects/brain_gwas/data/prefiltered_vcfs/{2}/{0}_{1}".format(gwas_chrom, gwas_pos, gwas_suffix.replace(".", "_")), shell=True)
		#command = 'vcftools --gzvcf /mnt/lab_data/montgomery/shared/1KG/ALL.chr{0}.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz --maf 0.01 --max-maf 0.99 --chr {0} --from-bp {1} --to-bp {2} --recode --recode-INFO-all --out /users/mgloud/projects/brain_gwas/data/prefiltered_vcfs/{4}/{0}_{3}/{0}_{3}_{5}_prefiltered'.format(gwas_chrom, gwas_pos-window, gwas_pos + window, gwas_pos, gwas_suffix.replace(".", "_"), use_prefiltering)
		#subprocess.check_call(command, shell=True)

	# Use VCFtools to filter down to appropriate sites
	#command = 'vcftools --vcf /users/mgloud/projects/brain_gwas/data/prefiltered_vcfs/{0}/{1}_{2}/{1}_{2}_{6}_prefiltered.recode.vcf --positions /users/mgloud/projects/brain_gwas/tmp/vcftools/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}.txt --recode --recode-INFO-all --out /users/mgloud/projects/brain_gwas/tmp/plink/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_1Kgenomes'.format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering)
	command = 'vcftools --vcf /users/mgloud/projects/brain_gwas/data/prefiltered_vcfs/{0}/{1}_{2}/{1}_{2}_prefiltered.recode.vcf --positions /users/mgloud/projects/brain_gwas/tmp/vcftools/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}.txt --recode --recode-INFO-all --out /users/mgloud/projects/brain_gwas/tmp/plink/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_1Kgenomes'.format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering)
        subprocess.check_call(command, shell=True)
	...
	with open('/users/mgloud/projects/brain_gwas/tmp/plink/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_1Kgenomes.recode.vcf'.format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering)) as f:
		for line in f:
			if line.startswith("#"):
				continue
			else:
	...
	with open('/users/mgloud/projects/brain_gwas/tmp/plink/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_1Kgenomes.matched.recode.vcf'.format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering), "w") as w:
		with open('/users/mgloud/projects/brain_gwas/tmp/plink/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_1Kgenomes.recode.vcf'.format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering)) as f:
			for line in f:
				if line.startswith("#"):
					w.write(line)
	...
	command = '''/srv/persistent/bliu2/tools/plink_1.90_beta3_linux_x86_64/plink --vcf /users/mgloud/projects/brain_gwas/tmp/plink/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_1Kgenomes.matched.recode.vcf --keep-allele-order --make-bed --double-id --out /users/mgloud/projects/brain_gwas/tmp/plink/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_1Kgenomes_plinked'''.format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering)
	subprocess.check_call(command, shell=True)

	# Use PLINK to generate LD score
	command = '''/srv/persistent/bliu2/tools/plink_1.90_beta3_linux_x86_64/plink -bfile /users/mgloud/projects/brain_gwas/tmp/plink/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_1Kgenomes_plinked --r square --out /users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}'''.format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering)
	subprocess.check_call(command, shell=True)

	# Fix LD-score by replacing nan values with 0.
	# TODO: Verify that this is valid and doesn't screw up results.
	subprocess.check_call("sed s/nan/0/g /users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}.ld > /users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}.fixed.ld".format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering), shell=True)
	...
	with open("/users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_eqtl.z".format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering), "w") as w:
		snps = combined[['snp_pos', 'ZSCORE_eqtl']]
		snps.to_csv(w, index=False, header=False, sep="\t")

 	with open("/users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_gwas.z".format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering), "w") as w:
	...
	command = '''/srv/persistent/bliu2/tools/caviar/CAVIAR-C++/eCAVIAR \
		-o /users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_ecaviar_results \
		-l /users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}.fixed.ld \
		 -l /users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}.fixed.ld \
		 -z /users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_gwas.z \
		  -z /users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_eqtl.z \
		-c 1 \
		-r 0.95 > /dev/null'''.format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering)
	subprocess.check_call(command, shell=True)

	# Parse eCAVIAR results to compute CLPP score
	command = '''awk '{{sum += $2}} END {{print sum}}' /users/mgloud/projects/brain_gwas/tmp/ecaviar/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_{6}_ecaviar_results_col'''.format(gwas_suffix.replace(".", "_"), gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, use_prefiltering)
	clpp = float(subprocess.check_output(command, shell=True))

	with open("{0}/{1}_clpp_status_prefiltering_test.txt".format(base_output_dir, gwas_suffix.replace(".", "_")), "a") as a:
		a.write("{0}_{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n".format(gwas_chrom, gwas_pos, tissue_prefix, gene, conditional_level, snps.shape[0], clpp, used_prefiltering))

	# TODO: Warning, if run many times this script will make a lot of junk in the tmp directories.
	# If necessary, modify the code to clean up these files after use.
