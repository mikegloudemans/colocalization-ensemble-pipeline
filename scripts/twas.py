...
def run_twas(locus, window=500000):
    ...
    if "model" not in locus.settings["eqtl_experiments"][locus.eqtl_file]: 
        ...
        with open("{6}/twas/{0}/{1}_{2}/{3}/{4}_fastqtl_level{5}_twas_weights.pos".format(locus.gwas_suffix, locus.chrom, locus.pos, locus.eqtl_suffix, locus.gene, locus.conditional_level, locus.tmpdir), "w") as w:
            ...
            w.write("{0}\t{1}\t{2}\t{3}\t{4}\n".format("{4}_fastqtl_level{5}_twas_weights.wgt.RDat".format(locus.gwas_suffix, locus.chrom, locus.pos, locus.eqtl_suffix, locus.gene, locus.conditional_level, locus.tmpdir), "nameless_gene", locus.chrom, locus.pos, locus.pos))
    ...
    if "rsid_gwas" in combined.columns.values:
        combined['rsid'] = combined['rsid_gwas']
    if "ref_gwas" in combined.columns.values:
        combined['non_effect_allele'] = combined['ref_gwas']
    if "alt_gwas" in combined.columns.values:
        combined['effect_allele'] = combined['alt_gwas']
